...
const SideBar: React.FC<SideBarProps> = ({ ingredients, onDragStart, edges, setEdges }) => {
  ...
  return (
    <Toolbar.Root className="h-5/6 w-[15%] absolute top-1/2 -translate-y-1/2 right-5 bg-white rounded-2xl shadow-lg border border-zinc-300 flex flex-col items-center justify-start gap-2 py-5">
      ...
      {ingredients.map((ingredient, index) => (
        <Toolbar.Button
          ...
          draggable
          className={`w-10/12 h-auto p-2 top-10 right-0 border border-gray-300 transition-all duration-300 
                      ${ingredients[index].type === 'circle' ? 'rounded-full bg-gray-200' : 'bg-white rounded-tl-lg '} hover:-translate-x-4 hover:scale-105 text-black`}
        >
          {ingredient.label}
